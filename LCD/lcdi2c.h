...
static int process_options(int argc, char **argv)
{
    ...
    while( (option = getopt(argc, argv,"ilha:r:c:x:y:b:s:")) != -1) {
        switch (option) {
            ...
            case 'i': opt_init=1;
                break;
            case 'c': 	opt_cols=str2int8('c',optarg,10,0,20);
                if(opt_cols<0) return  -1;
                break;
            case 'r': opt_rows=str2int8('r',optarg,10,0,4);
                if(opt_rows<0) return  -1;
                break;
            case 'a': opt_address=str2int8('a',optarg,16,0,127);
                if(opt_address<0) return  -1;
                break;
            case 'x': opt_x=str2int8('x',optarg,10,0,19);
                if(opt_x<0) return  -1;
                break;
            case 'y': opt_y=str2int8('y',optarg,10,0,3);
                if(opt_y<0) return  -1;
                break;
            case 'b': opt_backlight=str2int8('b',optarg,10,0,2);
                if(opt_backlight<0 ) return  -1;
                break;
            case 's': opt_cursor=str2int8('c',optarg,10,0,3);
